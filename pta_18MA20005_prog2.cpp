...
In order to run the above triangulation algorithm, we First need to
...
hypothesis. 
We defne a reflex vertex to be a vertex of the polygon whose interior angle is at least pi, and
otherwise the vertex is nonreflex. We defne a reflex chain to be a sequence of one or more
...
additional diagonal in constant time. Since any triangulation consists of n^2 diagonals, the
